...
def generate(state: dict) -> str:
    ...
    if tailscale.get("enabled"):
        lines += [
            ...
            # Subnet-router SNAT: without this, a LAN client's traffic forwarded
            # onto the tailnet keeps its real LAN source IP, which a remote
            # subnet router's ACL/routing has no reason to accept or return to.
            # Masquerading to the device's own Tailscale IP makes every accepted
            # subnet route reachable from the LAN, matching how the WAN
            # masquerade works for internet traffic. This must be generated by
            # us (not left to Tailscale's own --snat-subnet-routes) because the
            # top-of-script "$IPT -t nat -F" flush wipes Tailscale's rule too,
            # and it's only restored on Tailscale's own reconcile cycle.
