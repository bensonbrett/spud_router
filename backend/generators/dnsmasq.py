...
def generate(state: dict) -> str:
    ...
    # Whether dnsmasq serves DHCP on mgmt at all (#213). Default True
    # preserves pre-#213 behavior; False for a dedicated/VLAN mgmt
    # interface joining an EXISTING management network's own DHCP server
    # (mgmt_addr_mode="dhcp" — spud-router must not compete with it).
    ...
    if mgmt_enabled and "." not in mgmt_if and mgmt_dhcp_server:
        lines += [
    ...
    for vlan in vlans:
        ...
        # A tagged management VLAN (#207's "vlan" mode) is this loop's ONLY
        # source of DHCP for its own interface (the dedicated "mgmt block"
        # above never fires for a dotted mgmt_if) — so mgmt_dhcp_server must
        # be honored here too, not just in that block, or turning it off
        # would have no effect for this mgmt topology (#213).
        if mgmt_enabled and subif == mgmt_if and not mgmt_dhcp_server:
            continue
        gw    = vlan["ip_address"]
