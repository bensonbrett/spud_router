...
- **Nebula** — join-only overlay mesh: import a pre-signed host cert/key + CA cert (validated with `nebula-cert verify` and a live smoke test before anything is saved), lighthouse hosts and static host map, **relay support** (route through a relay — typically the lighthouse — when two peers can't punch a direct tunnel; nebula upgrades to direct automatically once it forms), and its own inbound/outbound overlay firewall rules (separate from the WAN-facing iptables rules).
- Nebula firewall rules select either a single overlay host (or any host) or a list of certificate groups. Group selectors are mutually exclusive with a host selector.
